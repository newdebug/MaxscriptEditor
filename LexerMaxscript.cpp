...
LexerMaxscript::LexerMaxscript(QObject *parent) :
    QsciLexerCPP(parent)
{
}

LexerMaxscript::~LexerMaxscript()
{

}

const char *LexerMaxscript::language() const
{
    return "C++";
}

const char *LexerMaxscript::lexer() const
{
    return "cpp";
}

QStringList LexerMaxscript::autoCompletionWordSeparators() const
{
    QStringList wl;
...
const char *LexerMaxscript::blockStartKeyword(int *style) const
{
    if (style)
        *style = Keyword;
...
const char *LexerMaxscript::blockStart(int *style) const
{
    if (style)
        *style = Operator;
...
const char *LexerMaxscript::blockEnd(int *style) const
{
    if (style)
        *style = Operator;
...
int LexerMaxscript::braceStyle() const
{
    return Operator;
}

const char *LexerMaxscript::wordCharacters() const
{
    return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_#$";
}

const char *LexerMaxscript::keywords(int set) const
{
    if (set == 1)
        qDebug() << "set 1";
        return
